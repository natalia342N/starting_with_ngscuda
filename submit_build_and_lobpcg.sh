...
echo "=== Building ==="
cd "$WORKING_DIR/build/ngsolve/ngsolve"
make -j4 install 2>&1 | grep -E "error:|nvcc|nvcpp|Installing ngscuda|make\[.*Error" | tail -30

echo "=== GPU LOBPCG test ==="
cd "$WORKING_DIR"
python test_lobpcg_gpu.py
echo "=== Done ==="
